...
from app.deps import BlockAiDep, DbDep, RoundAiDep
...
def generate_report(db: DbDep, ai: RoundAiDep, round_id: int) -> ReportOut:
...
def tone_samples(db: DbDep, ai: BlockAiDep, block_id: int) -> ToneSamplesOut:
    return reports.tone_samples_for_block(db, ai, block_id)


@router.post("/blocks/{block_id}/critique", response_model=CritiqueOut)
def critique(db: DbDep, ai: BlockAiDep, block_id: int) -> CritiqueOut:
